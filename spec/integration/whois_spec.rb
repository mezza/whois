    ...
    it "queries and returns a record" do
      with_definitions do
        ...
        expect(Whois::Server::Adapters::Standard.query_handler).to receive(:call)
          .with("example.it", "whois.nic.it", 43, timeout: 10)
          .and_return(response)
  ...
  describe "Passing :bind_host and :bind_port options" do
    it "binds the WHOIS query to given host and port" do
      with_definitions do
        Whois::Server.define(:tld, "it", "whois.nic.it")
        expect(Whois::Server::Adapters::Standard.query_handler).to receive(:call)
          .with("example.it", "whois.nic.it", 43, "192.168.1.1", 3000, timeout: 10)
          .and_return(response)
  ...
  describe "Passing :bind_port options" do
    it "binds the WHOIS query to given port and defaults host" do
      with_definitions do
        Whois::Server.define(:tld, "it", "whois.nic.it")
        expect(Whois::Server::Adapters::Standard.query_handler).to receive(:call)
          .with("example.it", "whois.nic.it", 43, Whois::Server::Adapters::Base::DEFAULT_BIND_HOST, 3000, timeout: 10)
          .and_return(response)
  ...
  describe "Passing :host options" do
    it "forces the WHOIS query to given host" do
      with_definitions do
        Whois::Server.define(:tld, "it", "whois.nic.it")
        expect(Whois::Server::Adapters::Standard.query_handler).to receive(:call)
          .with("example.it", "whois.example.com", 43, timeout: 10)
          .and_return(response)
