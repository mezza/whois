  ...
  describe "#query_the_socket" do
    context "without :bind_host or :bind_port options" do
      ...
      it "does not bind the WHOIS query" do
        expect(described_class.query_handler).to receive(:call).with("example.test", "whois.test", 43, timeout: nil)
    ...
    context "with :bind_host and :bind_port options" do
      ...
      it "binds the WHOIS query to given host and port" do
        expect(described_class.query_handler).to receive(:call).with("example.test", "whois.test", 43, "192.168.1.1", 3000, timeout: nil)
    ...
    context "with :bind_port and without :bind_host options" do
      ...
      it "binds the WHOIS query to given port and defaults host" do
        expect(described_class.query_handler).to receive(:call).with("example.test", "whois.test", 43, described_class::DEFAULT_BIND_HOST, 3000, timeout: nil)
